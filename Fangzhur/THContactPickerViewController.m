...
- (void)viewDidLoad
{
    ...
    barButton = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(done:)];
    self.navigationItem.rightBarButtonItem = barButton;
    
    // Initialize and add Contact Picker View
    self.contactPickerView = [[THContactPickerView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 100)];
    self.contactPickerView.delegate = self;
    [self.contactPickerView setPlaceholderString:@"搜索联系人"];
    [self.view addSubview:self.contactPickerView];
    
    // Fill the rest of the view with the table view
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.contactPickerView.frame.size.height, SCREEN_WIDTH, self.view.frame.size.height - self.contactPickerView.frame.size.height - kKeyboardHeight) style:UITableViewStylePlain];
...
//    }
    ...
    self.textFieldCell.textField.text = user.phone;
    if (user.phone) {
        [self.textFieldCell.cacheArray replaceObjectAtIndex:self.textFieldCell.tag withObject:user.phone];
    }
...
{
    NSLog(@"%@",     personViewController.displayedProperties);
    return YES;
}
